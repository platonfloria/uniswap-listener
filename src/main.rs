    ...
    async fn listen_to_events(&self, from_block: Block<H256>, log_sender: Sender<(SystemTime, Log)>) -> web3::contract::Result<()> {
        ...
        sub.fold(log_sender, |log_sender, log| async move {
            if let Ok(log) = log {
                ...
                log_sender.send((timestamp, log)).await;
            }
    ...
    async fn process_logs(&self, mut log_receiver: Receiver<(SystemTime, Log)>) -> web3::contract::Result<()> {
        loop {
            if let Some((timestamp, log)) = log_receiver.recv().await {
                ...
                let (pool_type, symbol, price) =  match hex::encode(log.topics[0].as_bytes()).as_str() {
...
New price: {}",
                    ...
                    block_number,
                    hex::encode(transaction_hash.as_bytes()).as_str(),
                    hex::encode(pool_contract_address.as_bytes()).as_str(),
                    pool_type,
                    symbol,
                    price
                );
    ...
    async fn get_uniswap_v2_pool_info(&self, address: Address) -> web3::contract::Result<(String, u32, u32)> {
        ...
        Ok((format!("{}/{}", symbol0, symbol1), decimals0, decimals1))
    }
